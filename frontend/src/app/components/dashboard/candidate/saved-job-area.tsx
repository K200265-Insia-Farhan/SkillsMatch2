    ...
    const fetchSavedJobs = async () => {
      try {
        ...
        const response = await axios.get(
          `http://a5fc17b29b02841ce84a49e511820394-1637568402.us-east-1.elb.amazonaws.com:5000/api/auth/getAllSavedJobs`,
          {
            headers: {
    ...
    const fetchJobDetails = async () => {
      const promises = savedJobs.map(async (savedJob) => {
        try {
          const token = localStorage.getItem("token");
          const response = await axios.get(
            `http://a5fc17b29b02841ce84a49e511820394-1637568402.us-east-1.elb.amazonaws.com:5000/api/auth/getJobDetailsUsingSavedJobs/${savedJob.job_id}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
